...
Future<List<DetalleModel>> fetchDetalles() async {
  final accessToken = await storage.read(key: 'access_token');
  final tokenType = await storage.read(key: 'token_type');
  final response = await http.get(
    Uri.parse('${dotenv.env['API_URL']}/api/detalles'),
    headers: {
  ...
  if (response.statusCode == 200) {
    ...
    final List<DetalleModel> products =
        data.map((e) => DetalleModel.fromJson(e)).toList();
    return products;
  } else {
